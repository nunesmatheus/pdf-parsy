...
def convert(path):
    file = open(path, 'rb')
    text = __process_pages(file)
    file.close()
    return text


def __process_pages(file):
    retstr = StringIO()
    resource_manager = PDFResourceManager()
    interpreter = __interpreter(resource_manager, retstr)
...
def __interpreter(resource_manager, retstr):
    device = TextConverter(resource_manager, retstr, codec='utf-8',
                           laparams=__layout_params())
